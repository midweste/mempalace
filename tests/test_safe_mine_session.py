...
# ── Integration: mine() uses safe_mine_session ──────────────────────────


def _make_project(root: Path) -> Path:
    """Create a minimal minable project and return the project root."""
    (root / "backend").mkdir(parents=True, exist_ok=True)
    (root / "backend" / "app.py").write_text("def main():\n    print('hello')\n" * 20)
...
class TestMineIntegration:
    """Verify that mine() and mine_convos() use safe_mine_session."""

    def test_mine_calls_close_palace_via_session(self, tmp_path):
        """mine() must use safe_mine_session, which calls close_palace."""
        from mempalace.miner import mine

        project = _make_project(tmp_path / "project")
        palace = tmp_path / "palace"

        with patch("mempalace.palace.safe_mine_session.__exit__") as mock_exit:
            # Let the real __enter__ run but spy on __exit__
            mock_exit.return_value = False
            mine(str(project), str(palace))

        # __exit__ was called (proves with-block was used)
        mock_exit.assert_called_once()

    def test_mine_graceful_interrupt_produces_results(self, tmp_path):
        """mine() with simulated interrupt should stop but still have data."""
        from mempalace.miner import mine

        project = _make_project(tmp_path / "project")
        # Add more files so we can interrupt mid-way
        for i in range(10):
            (project / "backend" / f"mod{i}.py").write_text(
                f"# Module {i}\ndef func_{i}():\n    return {i}\n" * 20
            )

        palace = tmp_path / "palace"

        # Patch safe_mine_session to simulate interrupt after 2 files
        original_enter = safe_mine_session.__enter__

        def interrupt_after_2(self_obj):
            result = original_enter(self_obj)
            # We'll check interrupted in the loop; simulate by patching
            # the process_file to count calls and set interrupted
            return result
